...
class IntrafiExclusionListPageAsync private constructor(
    ...
    fun getNextPage(): CompletableFuture<Optional<IntrafiExclusionListPageAsync>> {
      return getNextPageParams().map {
        intrafiExclusionsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<IntrafiExclusion>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<IntrafiExclusionListPageAsync>>.forEach(action: (IntrafiExclusion) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
