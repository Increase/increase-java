...
class RealTimePaymentsTransferListPageAsync private constructor(
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<RealTimePaymentsTransfer>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<RealTimePaymentsTransferListPageAsync>>.forEach(action: (RealTimePaymentsTransfer) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
