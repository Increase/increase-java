...
class ProofOfAuthorizationRequestListPageAsync private constructor(
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<ProofOfAuthorizationRequest>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ProofOfAuthorizationRequestListPageAsync>>.forEach(action: (ProofOfAuthorizationRequest) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
