...
class OAuthApplicationListPageAsync private constructor(
    ...
    fun getNextPageParams(): Optional<OAuthApplicationListParams> {
      ...
      return Optional.of(OAuthApplicationListParams.builder().from(params).apply {nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<OAuthApplicationListPageAsync>> {
      return getNextPageParams().map {
        oauthApplicationsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<OAuthApplication>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<OAuthApplicationListPageAsync>>.forEach(action: (OAuthApplication) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
